...
훅은 **프로세스가 태어날 때의 페인 키**를 보고한다. 세션은 처음 뜬 터미널의 환경을 계속 물고
살기 때문에, 나중에 다른 터미널에서 그 세션을 붙여 보고 있어도 상태는 처음 터미널로 간다.
목록의 `3.3` 같은 번호는 페인 키에서 계산한 값이라 **번호와 페인 키를 대조해도 이 어긋남은 안
잡힌다**(같은 정보의 두 표현이다).

**증상으로 알아보는 법: 작업중 아이콘은 엉뚱한 터미널에 뜨는데 다 끝났다는 종은 제대로 뜬다.**
둘은 경로가 다르다 — 종은 터미널이 실제로 받은 BEL(pty 출력)이라 언제나 맞고, 작업중은 훅이
보낸 페인 키다. 이 비대칭이 보이면 페인 키 오배정이다.

잡히는 근거는 **터미널 자신의 기록**이다(`%APPDATA%/orca/terminal-history/<ptyId>/output.log`).
`shared/pane-binding-audit.ts`가 꼬리 512KB를 읽어 **그 턴이 화면에 찍은 문장**을 센다. 애매하면
아무것도 하지 않는다 — 두 터미널이 같이 걸림, 묶인 페인의 기록 없음, 자기 터미널에 이미 흔적 있음.

### 세션 id로는 못 찾는다

에이전트는 자기 세션 id를 화면에 찍지 않는다. 실측: 살아 있는 터미널 5개 중 3개의 기록에 UUID가
**한 번도** 없었다. id만 세던 첫 판은 그래서 아무것도 못 찾고 "이상 없음"을 띄웠다. 화면에
세션을 실어 나르는 이스케이프도 없다 — 기록에 남는 OSC는 창 제목(OSC 0) 하나뿐이고, 거기 든
것은 세션 id가 아니라 그 턴의 요약이다. `~/.claude/daemon/attach-journal`도 붙은 클라이언트의
pid만 적고 세션을 안 적는다.

### 무엇을 근거로 세는가

상태가 들고 있는 네 가지를 다 쓴다: `lastAssistantMessage`, `lastCompletedAssistantMessage`,
`prompt`, `toolInput`. **`lastAssistantMessage` 하나로는 안 된다** — 다음 턴이 시작되는 순간
비워지는데, 검사를 돌릴 만한 때가 바로 그 `working` 중이다(그래서 `lastCompleted…`가 따로 있다).

대조 전에 양쪽에서 **이스케이프 시퀀스와 공백을 전부** 지운다. TUI는 터미널 폭에서 줄을 강제로
끊는데 한국어·CJK는 그 자리에 공백이 없어 단어 한가운데가 갈린다. 공백을 "합치면" 원문에 없는
공백이 남으니 아예 지운다. 지운 문자열을 20자 창으로 잘라 하나라도 걸리면 그 터미널로 본다 —
다시 그린 거터가 어느 한 구간을 깨도 다른 창이 살아남는다.

### 정정은 페인이 아니라 세션에 건다

`resolveAgentPaneAuthorityKey(paneKey, sessionId)`가 `setAgentStatus` 첫 줄에서 갈아 끼운다.
**페인 별칭으로 고치면 안 된다** — 렌더러(`agent-pane-authority.ts`)와 메인(`server.ts`의
`legacyPaneKeyAliases`) 둘 다 페인 단위라, 1.1을 3.3으로 돌리면 1.1로 보고하는 다른 것까지
따라간다. 세션 결속일 때는 메인에 알리지도 않는다(메인 것도 페인 단위라서).

세션 결속은 **메모리에만** 있다. 앱을 다시 켜면 잊는다.
...
- **검사 규칙을 손댈 때**: 기준값은 전부 임의값이다 — 근거 최소 16자, 창 20자, 창 상한 24개,
  id만 셀 때의 최소 3회·2배 여유·근거 가중치 5. 오탐이 생기면 값을 흔들기 전에 "무엇을 근거로
  삼는가"를 먼저 본다. 값을 바꾸기 전에 실데이터로 한 번 돌려 볼 것 — 훅이 저장해 둔
  `%APPDATA%/orca/agent-hooks/last-status.json`과 기록 폴더만 있으면 앱 없이 재현된다.
