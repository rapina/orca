...
**그 세션의 전사 파일에서만 읽는다** (`providerSession.transcriptPath`). 상태가 들고 있는
`prompt`·`toolInput`·`lastAssistantMessage`는 **쓰면 안 된다.** 페인당 상태는 하나인데 그 필드들은
직전 업데이트 값이 그대로 남고, 한 페인 키를 여러 세션이 함께 쓰므로(백그라운드 잡 호스트가 거느린
세션은 전부 그 호스트를 띄운 터미널을 답한다) 세션 id는 A인데 문장은 B의 것일 수 있다. 그러면
검사가 B의 터미널을 **확신을 갖고** 가리킨다. 실제로 그렇게 엉뚱한 터미널로 옮겨진 적이 있다.

**한 턴만 걸리면 답하지 않는다.** 잡 목록(fleet)을 띄운 터미널은 세션마다 요약 한 줄씩을 들고
있어서, 턴 하나가 걸리는 모습은 "그 세션을 돌리는 터미널"이 아니라 "남의 세션을 나열한 터미널"과
똑같다. 실측: 한 세션의 최근 턴 문장이 터미널 여섯 곳에 흩어져 있었고(135·42·15·15·1·1회),
그중 실제로 그 세션을 그리던 터미널만 두 번째 턴까지 갖고 있었다. 그래서 **서로 다른 턴 2개**가
같은 터미널에 걸려야 후보로 인정한다.

두 판정 단은 **섞이면 안 된다.** 근거 단(턴 2개)이 비었을 때만 세션 id 단으로 내려가고, 그때는
id 횟수만 센다. 예전 판은 폴백에서 총점(=근거 가중치 포함)을 써서, 턴 하나짜리 근거가 id 단의
문턱을 빌려 통과했다 — 잡 목록이 이기는 경로가 정확히 그것이다.

대조 전에 양쪽에서 **이스케이프 시퀀스와 공백을 전부** 지운다. TUI는 터미널 폭에서 줄을 강제로
끊는데 한국어·CJK는 그 자리에 공백이 없어 단어 한가운데가 갈린다. 공백을 "합치면" 원문에 없는
공백이 남으니 아예 지운다. 지운 문자열을 20자 창으로 잘라 창 단위로 센다 — 다시 그린 거터가
어느 한 구간을 깨도 다른 창이 살아남는다.

찾아낸 문장은 결과에 실어 버튼 툴팁에 보여 준다. 잘못 짚었는지는 **무엇에 걸렸는지 봐야만**
알 수 있고, 옮기기는 되돌리기가 없다.
